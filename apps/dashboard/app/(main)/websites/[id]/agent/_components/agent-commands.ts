...
export const AGENT_COMMANDS: AgentCommand[] = [
    ...
    },
    {
        ...
        keywords: ["find", "insights", "opportunities", "improve", "recommendations"],
    },
    // Compare
    {
        id: "compare-periods",
...
export function filterCommands(query: string): AgentCommand[] {
    ...
    return AGENT_COMMANDS.filter((cmd) => {
        ...
        const matchesKeywords = cmd.keywords.some((kw) => kw.includes(normalizedQuery));

        return matchesCommand || matchesTitle || matchesDescription || matchesKeywords;
    });
}
