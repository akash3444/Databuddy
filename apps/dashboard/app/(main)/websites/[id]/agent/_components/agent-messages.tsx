...
export function AgentMessages({ messages, isStreaming = false }: AgentMessagesProps) {
...
function MessageBubble({ content, role, isFinished, isStreaming }: MessageBubbleProps) {
...
function UserAvatar() {
    return (
        <Avatar className="size-8 shrink-0 border">
            <AvatarImage src="" />
            <AvatarFallback className="bg-accent text-foreground/70 text-xs">U</AvatarFallback>
        </Avatar>
    );
}
...
function ArtifactPreview({ artifact }: ArtifactPreviewProps) {
    ...
    return (
        <button
            ...
            type="button"
        >
            <span className="capitalize text-foreground/60">{artifact.type}</span>
...
export function FileAttachment({ file }: FileAttachmentProps) {
    ...
    return (
        <div className="flex items-center gap-2 rounded border bg-muted/50 px-3 py-2">
            <PaperclipIcon className="size-4 shrink-0 text-muted-foreground" weight="duotone" />
            <span className="font-medium text-sm">{file.filename ?? "Unknown file"}</span>
        </div>
    );
}
