        ...
        <AnimatePresence mode="wait">
            ...
            >
                {isScrollable && canScrollLeft && (
                    <div className="pointer-events-none absolute bottom-0 left-0 top-0 z-20 w-6 bg-linear-to-r from-background to-transparent" />
                )}

                {isScrollable && canScrollRight && (
                    <div className="pointer-events-none absolute bottom-0 right-0 top-0 z-20 w-6 bg-linear-to-l from-background to-transparent" />
                )}
                ...
                >
                    {prompts.map((prompt, index) => (
                        ...
                        >
                            <Button
                                className="h-auto shrink-0 whitespace-nowrap rounded-full border bg-background px-3 py-1.5 text-xs font-normal text-foreground/60 hover:text-foreground"
                                disabled={isLoading}
        ...
        <div className="flex flex-wrap gap-2">
            {prompts.map((prompt, index) => (
                <motion.button
                    ...
                    className={cn(
                        "rounded-full border border-dashed px-3 py-1.5 text-xs text-foreground/70",
